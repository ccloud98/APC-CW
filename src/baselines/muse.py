    ...
    def compute_recos(self, test_dataloader, n_recos=500):
        ...
        with torch.no_grad():
            for batch_idx, xx_pad in enumerate(test_dataloader):
                ...
                # 한 번에 계산하지 않고 작은 배치로 분할
                batch_scores_list = []
                chunk_size = 32  # 매우 작은 배치로 처리
                for i in range(0, B, chunk_size):
                    end_idx = min(i + chunk_size, B)
                    sub_batch = batch_repr[i:end_idx]
                    
                    item_emb = self.model.backbone.item_embedding.weight
                    logits = torch.matmul(sub_batch, item_emb.T)
                    scores = torch.softmax(logits, dim=1)
                    batch_scores_list.append(scores)
                
                scores_batch = torch.cat(batch_scores_list, dim=0)
                
                # 상위 n_recos 아이템 추출
                _, top_inds = torch.topk(scores_batch, k=n_recos, dim=1)
                
                # CPU로 이동 후 넘파이로 변환
                top_inds = top_inds.cpu().numpy() - 1
